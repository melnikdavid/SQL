...
select c.CategoryName,
	   p.ProductName, 
       sum(p.UnitPrice) as unitprice_sum
from products p
INNER JOIN Categories c 
ON p.CategoryID=c.CategoryID
--group by GROUPING SETS ((),c.CategoryName,(c.CategoryName,p.ProductName))
...
select  case 
             when year(o.OrderDate) is NULL and month(o.OrderDate) is null  then ' Total :'
			 else CAST( year(o.OrderDate) AS varchar(4))
		end 	 as order_year,
		case 
             when year(o.OrderDate) is NOT NULL and month(o.OrderDate) is null  then concat(' Total  :',year(o.OrderDate))
...
-- UNIQUE - הערך חייב להיות חד חד ערכי
-- Primark Key (PK) - מפתח ראשי , האינדקס למציאת רשומה ומורכב משני האילוצים שלעיל
-- Foreign Key (FK) - לא יכול להיות ערך שלא קיים ב-PK של הטבלה שאיליו ה-FK מופנה
-- Check - בדיקה של הערך לפני עדכון שלר רשומה קיימת או הכנסה של רשומה חדשה
-- Default - ערך ברירת מחדל לרשומה
...
insert into  Users(Age,firstName,lastName)
values (22, 'Jhon','Joe')
...
insert into my_employees
  values ('Aviv Cohen','Clerk', 10, 4000)

  insert into my_employees(name,title,depid)
  values ('Aviv Cohen','Clerk', 10)
...
Create View V_Category_Products_Counted as
select c.CategoryName,
	   p.ProductName, 
       sum(p.UnitPrice) as unitprice_sum
from products p
...
select max(unitprice_sum)
from V_Category_Products_Counted
order by 1,2

drop view V_Category_Products_Counted

--------------------------- End of lesson 11 --------------------
-- Home Work Lesson 11
--1
-- שיעורי הבית באדיבות מרים :)
--צור את הטבלה הבאה: My_Employees
create table My_Employees
( ID int identity (1,1)primary key,
   empName varchar(50),
   Title varchar(50),
...
--1.1     הכנס את הרשומה הראשונה בלבד מהנתונים לדוגמא, ללא ציון רשימת העמודות בפקודת ה .INSERT

insert into MY_Employees
  values ('Aviv Cohen','Clerk', 10, 4000)

--1.2    .הכנס את הרשומה השנייה מהנתונים לדוגמא, הפעם, יש לציין במפורש את רשימת העמודות בפקודת ה INSERT

 insert into  MY_Employees (empName,title,Deptid,Salary)
  values ('Miriam Levi','Sales Manager', 20,3750)

--1.3  עובד מספר 2 תפקד מצוין החודש והוחלט להעלות את משכורתו ל- 4500 יש לבצע את השינוי בטבלה

update MY_Employees
set Salary = 4500
where Salary =3750

--1.4   הוחלט לשחרר את עובד מספר 1 , יש למחוק את הרשומה

DELETE from MY_Employees
where Name = 'Aviv Cohen'

/*2. כתבו סקריפט ב SQL ליצירת טבלת בשם ordersHistory, עמודות:
...
( OrderID  int  identity (1,1) primary key ,
  Customer nchar(5)  FOREIGN KEY REFERENCES customers (CustomerID),
  Employee int FOREIGN KEY REFERENCES employees(employeeid),
  OrderDate datetime,
  deletedDate datetime)
...
select * from ordersHistory
   ...
   /*3.2) כתבו view בשם usCustomers שיציג רק את הלקוחות מארה"ב,

ה view לא יציג את עמודות הטלפון והפקס.*/
...
--End of Home Work Lesson 11

--------------------------- Lesson 12 --------------------
-- CTE (common table expression)
-- Triggers
-- SET, IF, CASE, WHILE, GO TO
...
WITH Emp_CTE (FirstName,LastName,CITY)  
AS
...
WITH products_and_categories (ProductName, CategoryName, UnitPriceOver10) 
AS
    ( SELECT p.ProductName,
             c.CategoryName,
             p.UnitPrice
	  ...
	  ON     c.CategoryID = p.CategoryID
      WHERE  p.UnitPrice > 10.0)
SELECT ProductName, CategoryName, UnitPriceOver10
FROM  products_and_categories
where UnitPriceOver10 < 10

WITH products_and_categories 
AS
    ( SELECT p.ProductName,
             c.CategoryName,
             p.UnitPrice
...
WITH CTE AS (
			SELECT YEAR(OrderDate) as [Year],
				   OD.UnitPrice,
				   OD.Quantity
			FROM   Orders LEFT JOIN [Order Details] OD ON OD.OrderID = Orders.OrderID
           )
SELECT   [Year], 
...
--לקוח והשנים בהם הזמין ,תרגיל בית 1
WITH CTE_year 
AS
      (SELECT YEAR(orderdate) AS orderyear, 
	          customerid
       FROM orders )
-- שנת ההזמנה וכמות הלקוחות שהזמינו באותה השנה
SELECT   orderyear, 
         count(DISTINCT customerid) AS cust_count
FROM     CTE_year
GROUP BY orderyear;

-- הצג מטבלת Products את מספר המוצר, שם המוצר ומחיר יחידה עבור המוצרים אשר מחירם גבוה מהמחיר הממוצע ליחידה - – פתור באמצעות CTE
-- שאלה 1
-- ללא CTE
SELECT ProductName,
       ProductID,
...
from users
 ...
 DELETE from userDetails
		where userDetails.userID = (select userid from deleted);
...
insert into  dbo.userDetails(userDetailText,userID)
values ('aaa', 4)
...
select *
from users U inner join userDetails UD
...
-- דוגמא
DECLARE @Average_unit_price int = ( select avg(UnitPrice) from Products )
...
select P.ProductID,
       p.UnitsInStock,
	   p.UnitPrice,
	   (select avg(UnitPrice) from Products) as [Avg Unit Price],
	   p.UnitPrice - (select avg(UnitPrice) from Products) as [Price Minus Avreage Unit Price] 	   
from Products P
...
-- Can we right the query below with a better performance, how can we do that?
select  p.ProductID, 
        p.UnitsInStock,
		p.unitPrice,
		(select avg(UnitPrice) from Products) as [avg Unit Price],
		p.unitPrice - (select avg(UnitPrice) from Products) as [Price minus Average]
from Products p
where  p.unitPrice > (select avg(UnitPrice) from Products)
order  by p.unitPrice
...
--Stored Procedures
CREATE procedure [dbo].[AVG_PRICE] 
as
begin
Declare @AVGPrice Money = (select avg(UnitPrice) from Products)
Print '123'
select  p.ProductID, 
        p.UnitsInStock,
		p.unitPrice,
		 @AVGPrice as [avg Unit Price],
		p.unitPrice -  @AVGPrice as [Price minus Average]
from Products p
...
-- Procedures
CREATE PROCEDURE [dbo].[getBossNameAndEmployee]
	@boss int
AS
BEGIN
	IF @boss = 0
		begin 
				select 'NO Boss TO Show'
		end
	else
	    begin
		    select concat(firstname , ' ',lastname)
			from Employees 
			where ReportsTo = @boss
		end
END
GO
...
-- Functions
select  year(getdate()) - year(e.BirthDate) age,
        dbo.calcAge(e.BirthDate)
from Employees E

select year(getdate())


EXEC dbo.getBossNameAndEmployee 5
...
BEGIN
	DECLARE @AGE int;
	SET @AGE = (select year(getdate()) - year(@birthDate));
	return @AGE;
END
GO
...
-- יצירת טבלת Orders TEMP, ORDERS TEMP HISTORY
exec INIT_ORDERS_TEMP

select * from [orders temp] 
where orderid =10248

select * from [orders temp history]
where orderid =10248
...
------------------------------------- בסיס לתשובה לשאלה 2
CREATE TRIGGER [DELETE ORDERS TEMP] 
   ON  [dbo].[Orders Temp]
   INSTEAD OF DELETE  
AS 
BEGIN

      SET IDENTITY_INSERT dbo.[orders Temp History] ON;

	  BEGIN TRAN
	  insert into [orders Temp History] (OrderID ,CustomerID,EmployeeID,OrderDate,RequiredDate,ShippedDate,ShipVia,Freight,ShipName,ShipAddress,ShipCity,ShipRegion,ShipPostalCode,ShipCountry)
	  select *
	  from   [orders Temp]
	  ...
	  COMMIT TRAN 
	  SET IDENTITY_INSERT dbo.[orders Temp History] OFF;
END
GO
-------------------------------------------------
...
-- שאלה 3
DECLARE @Average_unit_price int = ( select avg(UnitPrice) from Products )
--SET    @Average_unit_price = ( select avg(UnitPrice) from Products )

select P.ProductID,
       p.UnitsInStock,
	   p.UnitPrice,
  	   @Average_unit_price as [Avg Unit Price],
	   p.UnitPrice - @Average_unit_price as [Price Minus Avreage Unit Price] 	   
from   Products P
where  p.UnitPrice >   @Average_unit_price
order by P.UnitPrice
...
group by o.OrderID
having sum(OD.UnitPrice) > 10
 
CReate procedure OrdersAboveThreshold 
                  @Threshold,
				  @StartYear,
				  @EndYear
AS
...
-- פונקציות חלון
-- מאפשרות לקבל תוצאות אגרגטביות ללא שימוש ב- Group By
...
select  c.CategoryName,
        P.ProductName,
		P.unitprice,
		count(*) over (partition by  c.CategoryName ),
...
select concat  (e.FirstName, ' ',e.LastName) as [full name],
       e.country,
	    e.city,
       count(*) over (partition by e.country) as [Count Country],
       count(*) over (partition by e.country, e.city) as [Count Country City]
from Employees e
...
SELECT       YEAR(o.orderdate) AS "Year", 
             ...
             sum(od.unitprice) OVER(PARTITION BY YEAR(orderdate)
			                        order by YEAR(orderdate)) as [SUM Unit Price Per Year]
...
--ROW
--שורת ה- ROWS מאפשרת להציג מידע מחושב מצטבר עפ"י סדר המיון.

SELECT       YEAR(o.orderdate) AS "Year", 
             unitprice,
             sum(od.unitprice) OVER(PARTITION BY YEAR(orderdate) 
			                        order by YEAR(orderdate)
									--שורות בין השורה הקודמת והנוכחית ללא גבולות
...
select [ProductName],
       [UnitPrice],
	   [CategoryID],
	   sum(UnitPrice) over (partition by  [CategoryID] ) as  'sum of unit price'
from products
order by CategoryID
...
SELECT productid,
	   ...
	   unitprice,
	   [CategoryID],
       RANK() OVER(ORDER BY unitprice DESC) AS pricerank1,
       DENSE_RANK() OVER(ORDER BY unitprice DESC) AS pricerank2,
	   ROW_NUMBER () over(PARTITION BY [UnitPrice] order by  unitprice) as row_num,
	   NTILE (10) over(order by unitprice desc) as 'kvutzot'
FROM Products
ORDER BY pricerank1;
...
Insert into cars values ('car-1',100)
Insert into cars values ('car-2',200)
Insert into cars values ('car-3',300)


Declare @count int = 1
While @count <=10
Begin
      Insert into cars values ('car-'+convert(varchar,@count),100*@Count)
      Set @count = @count + 1 
end

-- צור לוח שנה מהיום ל-360 יום קדימה, שמור את הנתונים הטבלה עם עמודת תאריך
...
DECLARE @EndDate DATETIME
SET @StartDate = GETDATE()
SET @EndDate = DATEADD(d, 0, @StartDate)

EXEC [Set Calender]  @StartDate  ,  @EndDate


-- Cursors

--> Declare --> Open --> Fetch --> If The cursor is not Empty then Fetch Next -->Fetch Next and check again if Empty
                                --  Else  (Cursor is Empty) Close Cursor and Deallocate the cursor
...
FOR select p.ProductName,
           p.UnitPrice
	from products p
-- Open The Cursor
OPEN cursor_product;
--Then, fetch each row from the cursor and print out the product name and unit price:
FETCH NEXT FROM cursor_product INTO 
    @product_name, 
    @unit_price

WHILE @@FETCH_STATUS = 0 -- Check Status in the condition of the wile loop
    BEGIN
	    SET @TotalProducts = @TotalProducts +1
        PRINT concat(@product_name,' ', @unit_price);
        FETCH NEXT FROM cursor_product INTO 
            @product_name, 
            @unit_price;
       -- @TotalProducts = @TotalProducts +1 
    END;
print concat('The Total Product is :',   @TotalProducts)
...
Pivot(sum(total) for MM in ( [1] , [2] , [3])) as pvt
...
select City , [Customers] , [Suppliers]
from
        (
		SELECT        City,
					  Relationship,
					  count(Relationship) total_rel
		FROM          [Northwind].[dbo].[Customer and Suppliers by City]
		group by      City,
					  Relationship
		) as cust_Supplier
Pivot (sum(total_rel) for  Relationship in ( [Customers] , [Suppliers])) as Pvt 


select Relationship , [Sao Paulo] , [Buenos Aires] , [London]
from
        (
		SELECT        City,
					  Relationship,
...
Pivot (sum(total_rel) for  City in ( [Sao Paulo] , [Buenos Aires] , [London])) as Pvt 
...
select YYYY,CountMonth
from 
(
	select *
	from [Pivot Table]
) as pvt
unpivot ( CountMonth  for bla in ([1],[2],[3])) as unPvt
