...
-- תרגיל אקסל
select year(currOrder.OrderDate) as [Order Year],
       count(currOrder.CustomerID) as [CurNumCusts],
       (select count(prevOrder.CustomerID)
	    from    Orders prevOrder
	    where   year(prevOrder.OrderDate)  =   year(currOrder.OrderDate) - 1
		group by    year(prevOrder.OrderDate ) 
		) as [prevNumCats],
		count(currOrder.CustomerID)  -  (select count(prevOrder.CustomerID)
	                                     from    Orders prevOrder
	                                     where   year(prevOrder.OrderDate)  =   year(currOrder.OrderDate) - 1
		                                 group by    year(prevOrder.OrderDate ) 
										) as [Growth]
		
from Orders currOrder 
group by year(currOrder.OrderDate)
order by 1



select year(currOrder.OrderDate) as [Order Year],
       count(currOrder.OrderDate) as [CurNumCusts]
from Orders currOrder 
group by year(currOrder.OrderDate)
order by 1
