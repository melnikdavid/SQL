...
FROM products
where CHOOSE(categoryid ,1,2,3) is null
...
select e.Country,
	   ...
	   count(*) as  [Emp per Cities]
from   Employees e
GROUP BY GROUPING SETS(e.Country,(e.Country,e.city))
order by 1,2
...
with sales_amount_per_employees (Fullname, Year, Revenues) as
(
select FirstName + ' ' + LastName,
year(orderdate),
od.Quantity * od.UnitPrice * (1-Discount)
...
select Fullname, Year, sum(Revenues)

from sales_amount_per_employees

group by rollup (Fullname, Year)
order by Fullname
